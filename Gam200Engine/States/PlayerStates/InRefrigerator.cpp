...
InRefrigerator::InRefrigerator()
    : ptrString(nullptr), wasInputDetected(false), timer(0.f),randomRespawnSound(false)
{
}
...
void InRefrigerator::Execute(Player* obj, float dt)
{
    if (wasInputDetected == false)
    {
        ...
        {
            if (randomRespawnSound == false)
            {
                SceneManager::GetSceneManager()->GetCurrentScene()->GetSoundManager().Play_Sound(RESPAWN_SOUND1);
                randomRespawnSound = true;
            }
            else if (randomRespawnSound == true)
            {
                SceneManager::GetSceneManager()->GetCurrentScene()->GetSoundManager().Play_Sound(RESPAWN_SOUND2);
                randomRespawnSound = false;
            }
