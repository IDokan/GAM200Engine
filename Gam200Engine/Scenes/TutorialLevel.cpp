...
{	
    ...
    if (player1->GetComponentByTemplate<Physics>()->IsCollideWith(cheese) || player2->GetComponentByTemplate<Physics>()->IsCollideWith(cheese))
    {
        Graphics::Color4f color = cheese->GetComponentByTemplate<Sprite>()->GetColor();
        color.alpha = 0.f;
        cheese->GetComponentByTemplate<Sprite>()->SetColor(color);
    }
...
{
    fileIO* fileio = new fileIO;
    fileio->Output("../assets/fileIO/savedTutorial.txt");
}
